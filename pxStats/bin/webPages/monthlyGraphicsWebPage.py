...
def getMonths():
    ...
    for i in range(0,NB_MONTHS_DISPLAYED):
        ...
        if  currentDate.month -i < 1:
            year  = currentDate.year - int( abs(math.floor( float( ( currentDate.month - i - 1   ) / 12 ) ) ) ) 
        ...
        print "%s-%s-%s 00:00:00" %(year,month,"01")
    months.reverse()
...
def getStartEndOfWebPage():
    ...
    nbMonthsToRewind = NB_MONTHS_DISPLAYED - 1 
        
    if currentDate.month - (nbMonthsToRewind%12) < 1 :            
        month = currentDate.month - (nbMonthsToRewind%12)+12 
    
    if  currentDate.month -nbMonthsToRewind < 1:
        year  = currentDate.year - int( abs(math.floor( float( ( currentDate.month - nbMonthsToRewind -1  ) / 12 ) ) ) ) 
    
    else :                 
        month = currentDate.month - nbMonthsToRewind             
        year = currentDate.year
