...
def showPresentation():
    ...
    print "Create by Nicholas Lemay"
...
def updatePickleFiles( infos ):
    ...
    updateLogFiles = askUserAboutUpdatingLogs()
    
    if updateLogFiles == True :
        updateLogFiles()
...
def backupCurrentTimesOfPickleUpdates():
    ...
    print "cp -r %s %s.backup" %( StatsPaths.STATSPICKLESTIMEOFUPDATES, StatsPaths.STATSPICKLESTIMEOFUPDATES )
    #print output 
...
def askUserWhichDbBackupsToUse( choiceOfBackups ):
    ...
    if input == 'q':
        ...
        backupToUse = choiceOfBackups( int(input) )
...
def getThreeClosestDatabasesBackups( infos ):
    """
        @summary : Returns the three databases backups
                  ...
                  aske for the database recollection.
    """
